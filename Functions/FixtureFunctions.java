    ...
    public static Team[][] nextWeekPositionCreate(Team[][] Teams,Teams LastTeam){
        ...
        for(int x=0;x<Teams.length;x++){
            for(int y=0;y<Teams[x].length;y++){
                try{
                  if(Teams[(y==1) ? x + 1 : x][(y==1) ? 0 : 1].getTakimName() == LastTeam) tempTeams[x][y] = Teams[x][y];
                  if(Teams[(y==1) ? x + 1 : x][(y==1) ? 0 : 1].getTakimName() == LastTeam) continue; // Fixtur Jokeri Kontrolü 
                  if( ((y==1) ? x + 1 : x) == Teams.length - 1 && ((y==1) ? 0 : 1) == 1) continue; // Hafta Sonu Kontrolü // bence bu kod gereksiz try catche düşecek çalışmayacak bile
